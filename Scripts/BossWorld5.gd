...
@export var shield_down_by_phase: Array[float] = [5.0, 3.0, 1.3, 0.9]

# Generic missiles
@export var normal_missile_interval_by_phase: Array[float] = [5.0, 3.0, 1.0, 0.50]

# Scatter missiles
@export var scatter_interval_by_phase: Array[float] = [999.0, 10.0, 5.0, 2.8]
...
# EMP volleys
@export var emp_interval_by_phase: Array[float] = [999.0, 12.0, 8.0, 4.0]

# Ion missiles / zones
@export var ion_interval_by_phase: Array[float] = [999.0, 999.0, 13.0, 7.0]
...
@export var fighter_spawn_interval_by_phase: Array[float] = [7.0, 5.0, 4.0, 2.0]
@export var bomber_spawn_interval_by_phase: Array[float] = [999.0, 8.0, 4.2, 2.0]
...
@export var laser_charge_duration_by_phase: Array[float] = [3.2, 2.8, 12.0, 10.0]
@export var laser_weak_point_hp_by_phase: Array[int] = [2, 3, 2, 4]
@export var laser_target_cannons_weight: float = 0.2

# Drag the visible sprite here in the inspector
...
# Random top-area movement
@export var move_margin_x: float = 150.0
@export var move_min_y: float = 70.0
@export var move_max_y: float = 210.0
...
func _ready() -> void:
	...
	if flash_sprite:
		...
		print("❌ Mothership flash sprite NOT found. Set flash_sprite_path in the inspector.")
...
func _phase_index() -> int:
	if phase_health_thresholds.size() >= 1 and health > phase_health_thresholds[0]:
		return 0
	if phase_health_thresholds.size() >= 2 and health > phase_health_thresholds[1]:
		return 1
	if phase_health_thresholds.size() >= 3 and health > phase_health_thresholds[2]:
		return 2
	return 3
...
func _apply_phase_tuning(force_restart_timers: bool) -> void:
	...
	if force_restart_timers:
		_restart_attack_timer(missile_timer, debug_enable_normal_missiles, true)
		_restart_attack_timer(scatter_timer, debug_enable_scatter_missiles, true)
		_restart_attack_timer(emp_timer, debug_enable_emp, true)
		_restart_attack_timer(ion_timer, debug_enable_ion, true)
		_restart_attack_timer(fighter_timer, debug_enable_fighters, true)
		_restart_attack_timer(bomber_timer, debug_enable_bombers, true)
	else:
		_restart_attack_timer(missile_timer, debug_enable_normal_missiles, false)
		_restart_attack_timer(scatter_timer, debug_enable_scatter_missiles, false)
		_restart_attack_timer(emp_timer, debug_enable_emp, false)
		_restart_attack_timer(ion_timer, debug_enable_ion, false)
		_restart_attack_timer(fighter_timer, debug_enable_fighters, false)
		_restart_attack_timer(bomber_timer, debug_enable_bombers, false)
	...
	if debug_enable_laser and laser_phase:
		...
		if not laser_charge_active:
			_restart_laser_cooldown(force_restart_timers)
	else:
		laser_timer.stop()


func _restart_attack_timer(timer: Timer, enabled: bool, randomize_initial_delay: bool = false) -> void:
	timer.stop()

	if not enabled:
		return

	var delay: float = max(0.01, timer.wait_time)
	if randomize_initial_delay:
		delay = randf_range(0.25, delay)

	timer.start(delay)


func _restart_laser_cooldown(randomize_initial_delay: bool = false) -> void:
	if is_dead:
		return
	...
	var delay: float = max(0.01, laser_timer.wait_time)
	if randomize_initial_delay:
		delay = randf_range(0.5, delay)

	laser_timer.start(delay)
...
func _play_hit_flash() -> void:
	if flash_sprite == null:
		print("❌ Cannot flash mothership: flash_sprite is null")
		return
